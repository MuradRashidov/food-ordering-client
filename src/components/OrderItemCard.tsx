...
export const OrderItemCard = ({order}:Props) => {
    ...
    const handleStatusChange = async (orderStatus:OrderStatus) => {
       await updateOrderStatus({orderId:order._id,status:orderStatus});
    ...
    <Card>
        <CardHeader>
            <CardTitle className="grid md:grid-cols-4 gap-4 justify-between mb-3">
                ...
                <div>
                    Total Cost
                    <span className="ml-2 font-normal">
                        {(order.totalAmount/100).toFixed(2)}AZN
                    </span>
                </div>
            </CardTitle>
        </CardHeader>
        ...
        <CardContent className="flex flex-col gap-6">
            ...
            <div className="flex flex-col space-y-1.5">
                <Label htmlFor="status">What is status of this order?</Label>
